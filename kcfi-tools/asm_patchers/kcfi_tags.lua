...
hcr_ids = {}
hcr_ids["__put_user_1"] = "00dead04"
hcr_ids["__put_user_2"] = "00dead04"
hcr_ids["__put_user_4"] = "00dead04"
hcr_ids["__put_user_8"] = "00dead04"
hcr_ids["__get_user_1"] = "00dead03"
hcr_ids["__get_user_2"] = "00dead03"
hcr_ids["__get_user_4"] = "00dead03"
hcr_ids["__get_user_8"] = "00dead03"
...
--- BUILD list of ids that we need
i = 1
while i <= #ids do
  os.execute("echo " .. ids[i] .. " >> ./build_tools/fs.txt")
  i = i + 1
end
...
--- load call ids
os.execute("./build_tools/asm_head_id ./build_tools/fs.txt > ./build_tools/cids.txt")
dbc = io.open("./build_tools/cids.txt", "r")
if not dbc then
  print("Problem with id outputs")
  os.exit()
end

-- pupulate call ids
for line in dbc:lines() do
  f, tag = string.match(line, "(.*);(.*)")
  file:write("#define KCFIc_" .. f .. " 0x" .. tag .. "\n")
end

--- load ret ids
os.execute("./build_tools/asm_id ./build_tools/fs.txt > ./build_tools/rids.txt")
dbr = io.open("./build_tools/rids.txt", "r")
if not dbr then
  print("Problem with id outputs")
  os.exit()
end
...
--- populate ret ids
for line in dbr:lines() do
  f, tag = string.match(line, "(.*);(.*)")
  if hcr_ids[f] then
    tag = hcr_ids[f]
  end
  file:write("#define KCFIr_" .. f .. " 0x" .. tag .. "\n")
end
...
for line in dbr:lines() do
  f, tag = string.match(line, "(.*);(.*)")
  if (string.match(f, "__mutex_unlock_slowpath")) then
    file:write("#define cfi_mutex_fastpath_unlock __mutex_fastpath_unlock_cfi")
    file:write("(&lock->base.count, __mutex_unlock_slowpath, 0x" .. tag .. ")\n")
    file:write("#define cfi_mutex_fastpath_unlock2 __mutex_fastpath_unlock_cfi")
    file:write("(&lock->count, __mutex_unlock_slowpath, 0x" .. tag .. ")\n")
  end
  if (string.match(f, "__mutex_lock_slowpath")) then
    file:write("#define cfi_mutex_fastpath_lock __mutex_fastpath_lock_cfi")
    file:write("(&lock->count, __mutex_lock_slowpath, 0x" .. tag .. ")\n")
  end
end
dbr:close()
dbc:close()
